...
# remove missing values, make one df per emotional level, and recode factor level if needed
# sadness
...
df_dur_sad <- df_dur_sad %>% mutate(sadness = as.numeric(as.character(sadness)))
df_dur_hap <- df_dur_hap %>% mutate(happiness = as.numeric(as.character(happiness)))
df_dur_bor <- df_dur_bor %>% mutate(boredom = as.numeric(as.character(boredom)))
################################################################################
...
res_p_pearson <- bind_rows(
  run_pcor(df_dur_clean, y="RME", x="sadness",   controls=controls_list$sadness,   method="pearson"),
  run_pcor(df_dur_clean, y="RME", x="happiness", controls=controls_list$happiness, method="pearson"),
  run_pcor(df_dur_clean, y="RME", x="boredom",   controls=controls_list$boredom,   method="pearson")
)

# Spearman (robuste à l’ordinal/monotone)
res_p_spearman <- bind_rows(
  run_pcor(df_dur_clean, y="RME", x="sadness",   controls=controls_list$sadness,   method="spearman"),
  run_pcor(df_dur_clean, y="RME", x="happiness", controls=controls_list$happiness, method="spearman"),
  run_pcor(df_dur_clean, y="RME", x="boredom",   controls=controls_list$boredom,   method="spearman")
)
